...
/* What the output is stamped with, and why it is not a date.
 *
 * It used to be `new Date()`. The stamp is inside the text --check compares, so
 * an artefact committed yesterday could never equal one built today: the gate
 * went red on the calendar, whether the pricing model had moved or not. A gate
 * that cries wolf daily is a gate people stop reading, and this one was unread
 * for a while behind an unrelated failure earlier in the same job.
 *
 * The spreadsheet's mtime was the obvious replacement and is WRONG for the same
 * reason in reverse: git does not record mtimes, so actions/checkout stamps
 * every file with the checkout time and CI computes a different value from the
 * one committed. It passed locally and failed on the runner, which is the worst
 * shape a gate check can have.
 *
 * A content hash of the source is the answer. It is deterministic, it travels
 * in git because it is derived from bytes that are in git, and it answers the
 * question the gate actually asks: does this committed table match this
 * spreadsheet. A date never answered that. */
const stamp = createHash('sha256').update(readFileSync(XLSX)).digest('hex').slice(0, 12);
 ...
 * number. Source "Whollar Pricing Model.xlsx" sha256:${stamp}.
 *
...
const coverage = [
  `whollar-estimate-bench.js coverage, source sha256:${stamp}`,
  ``,
