...
/* The build date, taken from the SPREADSHEET rather than from the clock.
 *
 * `new Date()` here made --check fail every day whether the source had moved
 * or not: the stamp is inside the compared text, so the artefact committed
 * yesterday can never equal one built today, and CI turned red on the calendar
 * rather than on the data. That is a gate people stop reading, and it was
 * masked for a while behind an unrelated failure earlier in the same job.
 *
 * The source's own mtime is the honest answer to "when was this built from":
 * it moves when the pricing model moves and only then, so a green check now
 * means the committed table really does match the spreadsheet. */
const stamp = new Date(statSync(XLSX).mtime).toISOString().slice(0, 10);
