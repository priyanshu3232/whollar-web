...
/* The reduction reads offered on the ticket minus 'none', which is the absence
   ...
   themselves. */
export var MIX_TYPES = ['member', 'promo', 'cash', 'own'];

export var MIX_TYPE_LABEL = {
...
export var LABEL_RE = /^[A-Za-z0-9][A-Za-z0-9 ,.'&-]{2,39}$/;
export var LABEL_BANNED = /today only|limited time|expires|last chance|hurry|act now|bundle|autopay|auto[- ]pay/i;
 ...
 * until it is.
 */
export function allocate(gapCents, tenthsList) {
 ...
 */
export function tierSnapshot(tier, rows, guar) {
  var t = tier || {};
